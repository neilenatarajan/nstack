...
_VENDORED="no"
if [ -d ".factory/skills/nstack" ] && [ ! -L ".factory/skills/nstack" ]; then
  if [ -f ".factory/skills/nstack/VERSION" ] || [ -d ".factory/skills/nstack/.git" ]; then
    _VENDORED="yes"
  fi
fi
...
`.factory/skills/nstack/`. Vendoring is deprecated. We will not keep vendored copies
...
> This project has nstack vendored in `.factory/skills/nstack/`. Vendoring is deprecated.
...
If A:
1. Run `git rm -r .factory/skills/nstack/`
2. Run `echo '.factory/skills/nstack/' >> .gitignore`
