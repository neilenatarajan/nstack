...
const SCRIPT = path.join(import.meta.dir, "..", "bin", "nstack-learnings-search");

describe("nstack-learnings-search injection prevention", () => {
  ...
  test("uses process.env for all user-controlled values", () => {
    ...
    expect(bunBlock).toContain("process.env.NSTACK_SEARCH_QUERY");
    expect(bunBlock).toContain("process.env.NSTACK_SEARCH_LIMIT");
    expect(bunBlock).toContain("process.env.NSTACK_SEARCH_SLUG");
    expect(bunBlock).toContain("process.env.NSTACK_SEARCH_CROSS");
  });

  test("env vars are set on the bun command line", () => {
    // The env vars must be passed to bun, not just set in the shell
    expect(script).toContain("NSTACK_SEARCH_TYPE=");
    expect(script).toContain("NSTACK_SEARCH_QUERY=");
    expect(script).toContain("NSTACK_SEARCH_LIMIT=");
    expect(script).toContain("NSTACK_SEARCH_SLUG=");
    expect(script).toContain("NSTACK_SEARCH_CROSS=");
  });
});
