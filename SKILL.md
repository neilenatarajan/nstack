...
description: |
  ...
  test a site, verify a deployment, dogfood a user flow, or file a bug with screenshots. (nstack)
allowed-tools:
  - Bash
  - Read
  - AskUserQuestion
...
~/.claude/skills/nstack/bin/nstack-timeline-log '{"skill":"nstack","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
If `PROACTIVE` is `false`: do NOT proactively invoke or suggest other nstack skills during
this session. Only run skills the user explicitly invokes. This preference persists across
sessions via `nstack-config`.
...
- User asks to upgrade nstack → invoke `/nstack-upgrade`
...
If the user opts out of suggestions, run `nstack-config set proactive false`.
If they opt back in, run `nstack-config set proactive true`.

# nstack browse: QA Testing & Dogfooding
