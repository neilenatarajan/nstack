...
description: |
  ...
  "let another agent use my browser", or "give browser access". (nstack)
...
~/.claude/skills/nstack/bin/nstack-timeline-log '{"skill":"pair-agent","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
Your nstack browser runs a local HTTP server. This skill creates a one-time setup key,
...
side panel if you have NStack Browser open."
...
credentials are written to `~/.openclaw/skills/nstack/browse-remote.json`.
...
to `~/.codex/skills/nstack/browse-remote.json`.
...
`~/.cursor/skills/nstack/browse-remote.json`.
