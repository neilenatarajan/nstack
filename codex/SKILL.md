...
description: |
  ...
  "codex challenge", "ask codex", "second opinion", or "consult codex". (nstack)
...
~/.claude/skills/nstack/bin/nstack-timeline-log '{"skill":"codex","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
```bash
~/.claude/skills/nstack/bin/nstack-review-log '{"skill":"codex-review","timestamp":"TIMESTAMP","status":"STATUS","gate":"GATE","findings":N,"findings_fixed":N,"commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
- **Detect skill-file rabbit holes.** After receiving Codex output, scan for signs
  that Codex got distracted by skill files: `nstack-config`, `nstack-update-check`,
  `SKILL.md`, or `skills/nstack`. If any of these appear in the output, append a
  warning: "Codex appears to have read nstack skill files instead of reviewing your
